...
export default function ManageUser() {
    ...
    const renderTableBody = () => {
        return data?.items.map((user, index) => {
            return (
                <tr key={index} className="border-b border-opacity-20 dark:border-gray-700 dark:bg-gray-900 hover:bg-gray-100 duration-300 cursor-pointer">
                    <td className="p-3">
                        <p>{user.taiKhoan}</p>
                    </td>
                    <td className="p-3">
    ...
    if (loading) {
        ...
        return (
            <div className="container p-2 mx-auto sm:p-4 dark:text-gray-100">
                ...
                <h2 className="mb-4 text-2xl font-semibold leadi">Quản lý người dùng</h2>
                <div className='flex justify-end mb-3'>
                    <div className='flex'>
                        <form onSubmit={formik.handleSubmit}>
                            <input type="text" name='keyword' onChange={formik.handleChange} placeholder='Tìm kiếm' className='p-2 border rounded-l-md focus:outline-blue-700' />
                            <button type='submit' className='bg-blue-600 p-2 font-medium text-white rounded-r-md hover:bg-blue-700 duration-300'
                            >Tìm kiếm</button>
                        </form>
                        <button className='bg-blue-600 p-2 ml-2 font-medium text-white rounded-md hover:bg-blue-700 duration-300'
