                            ...
                            <SwiperSlide key={movie.maPhim}>
                                <div className='border'>
                                    ...
                                    <div className='p-3 bg-white'>
                                        <h1 className='font-bold text-blue-600 hover:text-blue-700 hover:underline duration-300 cursor-pointer uppercase h-14 mb-3'
                                            onClick={() => navigate(`/detail/${movie.maPhim}`)}
                                        >{movie.tenPhim.slice(0, 25)}</h1>
                                        <p className='text-sm text-gray-500 mb-5' style={{ height: 50 }}>{movie.moTa.slice(0, 50)}</p>
                                        <div className='flex gap-3 items-center'>
                                            <button className='w-full p-2 bg-blue-600 text-white rounded-md font-medium hover:bg-blue-700 duration-300 hover:scale-105'
                                                onClick={() => navigate(`/detail/${movie.maPhim}`)}
                                            >Đặt vé</button>
                                        </div>
                                    </div>
